...
- [x] Context window + context engineering (2026-06-28): orchestrator keeps a 50-segment rolling buffer; ResponseService passes the last 30 to the LLM. Context engineering is folded into the detector's EXISTING LLM call (NO extra round-trip → latency unchanged): `LlmQuestionDetectionService` returns `{isQuestion, question (self-contained), contextSummary}`; `DetectedQuestion`/`LlmRequest` carry `contextSummary` through to `CloudflareProvider.buildMessages`, which leads with "Context: <summary>" + transcript + resolved question and asks for a domain-expert, specifics-grounded answer. Verified: vague "How much should we spend?" → "...on the new marketing campaign?" + summary "...Sara suggesting $300,000", first-token ~0.8-1.0s.
...
- [x] CD / installers (2026-06-26, M2): `.github/workflows/release.yml` builds on a `v*` tag. Per OS: backend single-exe (mac = universal arm64+x64 via `lipo`), static ffmpeg fetched + bundled, `flutter build {macos,windows} --release --dart-define=REAL_BACKEND=true` (so **no fakes ship** — real `WebSocketBackendClient` + real Cloudflare STT/LLM defaults), `scripts/bundle_{macos.sh,windows.ps1}` copy exe+ffmpeg into the bundle where `BackendLauncher` resolves them, packaged to `.dmg`/`.zip`, attached to a GitHub Release. The shipped app spawns the backend with `LLM_PROVIDER=cloudflare` + `TRANSCRIPTION_ENGINE=cloudflare` + `CF_GATEWAY_URL` (repo var) so it reaches Cloudflare tokenless. Unsigned (M6 later). NOT yet run — first `v*` tag is its first real test; static-ffmpeg URLs are the likeliest first-run breakage. Windows system-audio (WASAPI, M3) still not built.
- [x] Phase 3 protocol additions (2026-07-01, client→backend, mirrored TS+Dart): `kb.set` {content} sets the session Knowledge Base (injected into the LLM prompt as authoritative context); `response.mode` {mode: auto|manual}; `response.generate` {questionId} (manual ▶). Backend: `SessionOrchestrator` holds KB + mode + a bounded `pendingQuestions` map (retains detected question + context snapshot so manual generate rebuilds the exact prompt). UI: top-bar tab switcher (Dashboard | Knowledge Base), KB editor panel, Auto/Manual toggle in AI Response panel, ▶ play button per question in manual mode. All shared/cross-platform (no OS-specific code). Verified via 4 orchestrator integration tests (auto answers on detect; manual defers then answers on generate; KB flows to the request).
